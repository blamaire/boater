...
Bind-mount is de hele repo; bestanden die in de container ontstaan zijn root-owned op de Windows-host — dat is bewust (zie `docker/php/Dockerfile`).
...
Bij het bouwen of herbouwen van een admin-CRUD-beheerscherm: zie skill `crud-ui-patroon` (actie-iconen, inline-edit vs. modal, rijhoogte).
...
- **CI pijplijn** (`.github/workflows/ci.yml`): een falende `composer ci` lokaal = falende PR.
