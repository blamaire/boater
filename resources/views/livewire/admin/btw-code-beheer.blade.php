<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
    <p class="text-sm text-gray-500">
        BTW-codes: percentage, richting (af te dragen bij verkoop / voor te vorderen bij inkoop), de gekoppelde
        BTW-rekening en een geldigheidsperiode. Koppel een code aan een product via <a href="{{ route('admin.products.index') }}" class="text-rzvg-600 hover:underline">Producten</a>
    ...
    <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 space-y-4">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <label class="block text-sm">
                <span class="text-gray-600">Richting</span>
                <select wire:model="direction" class="mt-1 block w-full border-gray-300 rounded shadow-sm text-sm">
                    @foreach ($directions as $d)
                        <option value="{{ $d->value }}">{{ $d->label() }}</option>
                    @endforeach
                </select>
                @error('direction') <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
            </label>

            <label class="block text-sm">
                <span class="text-gray-600">BTW-rekening</span>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Richting</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($codes as $code)
                        ...
                        <tr wire:key="btw-code-{{ $code->id }}" @class(['bg-rzvg-50' => $editingId === $code->id])>
                            ...
                            <td class="px-4 py-2 text-gray-700 text-xs">{{ $code->direction->label() }}</td>
                            <td class="px-4 py-2 text-gray-500 text-xs whitespace-nowrap">{{ $code->ledgerAccount->code }} · {{ $code->ledgerAccount->name }}</td>
                            ...
                            <td class="px-4 py-2 text-right whitespace-nowrap">
                                <button type="button" wire:click="edit({{ $code->id }})" class="text-rzvg-600 hover:text-rzvg-800 text-xs">Bewerken</button>
